...
const App: React.FC = () => {
  ...
  const [processosLista, setProcessosLista] = useState<IProcesso[]>([]);
  ...
  const [novaListaProcessos, setNovaListaProcessos] = useState<IProcesso[]>([]);
  const [algoritmoSelecionado, setAlgoritmoSelecionado] =
    useState<ICondicao['metodo']>('FIFO');
  const [resultadoEscalonamento, setResultadoEscalonamento] = useState<
    number[][] | null
  >();
  ...
  const adicionarProcesso = () => {
    ...
    setProcessosLista((prevProcessosLista) => [
      ...prevProcessosLista,
      novoProcesso,
    ]);
  };
  ...
  const handleDelete = (id: number) => {
    const updatedCards = novaListaProcessos.filter(
  ...
  const handleRun = () => {
    ...
    if (escalonadorSelecionado) {
      ...
      const resultadoEscalonamento = new escalonadorSelecionado().escalonador(
        novosProcessos
      );
      ...
      if (ctx) {
        ...
        novaListaProcessos.forEach((processo, index) => {
          setTimeout(() => {
            ...
            ctx.fillRect(
              tempoAtual * 10,
              index * (barHeight + barMargin) + 20,
              processo.tempoExecucao * 10,
              barHeight
            );
            ctx.fillText(
              `Processo: ${processo.id}`,
              100 + tempoAtual * 10,
              index * (barHeight + barMargin) + 20 + barHeight / 2 + 5
            );

            const turnaround =
              1 + processo.tempoExecucao - processo.tempoChegada;
            ...
            if (index === novaListaProcessos.length - 1) {
              const turnaroundMedio =
                turnaroundTotal / novaListaProcessos.length;
              ctx.fillStyle = 'green';
              ctx.fillText(
                `Turnaround Médio: ${turnaroundMedio.toFixed(2)}`,
                300,
                20
              );
            }
      ...
      <main>
        <div className="firstSection">
          <Algoritmo
            ...
            processosLista={processosLista}
            setProcessosLista={setProcessosLista}
            setAlgortimoSelecionado={setAlgoritmoSelecionado}
          />
        </div>
                ...
                <div className="boxCard">
                  ...
                  <label htmlFor="" className="label">
                    ...
                    <input
                      ...
                      value={deadlineInput}
                      onChange={(e) =>
                        setDeadlineInput(parseInt(e.target.value))
                      }
                    />
                  </label>
                </div>
                <div className="boxCard">
                  <label htmlFor="" className="label">
                    ...
                    <input
                      ...
                      value={chegadaInput}
                      onChange={(e) =>
                        setChegadaInput(parseInt(e.target.value))
                      }
                    />
                  </label>
                  <label htmlFor="" className="label">
                    ...
                    <input
                      ...
                      value={paginasInput}
                      onChange={(e) =>
                        setPaginasInput(parseInt(e.target.value))
                      }
                    />
                  </label>
                </div>
                  ...
                  {novaListaProcessos.map((processo) => (
                    <div className="card2" key={processo.id}>
                      <div className='topSectionCard'>
                        <p className="idProcesso">Processo {processo.id}</p>
                        <button
                          className="exit"
